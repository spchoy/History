...
class ChatReadRetrieveReadApproach(Approach):
    ...
    """
#     system_message_chat_conversation = """Assistant helps with information related to the Port Authority of New York and New Jersey infrastructure, contracts, frequently asked questions (FAQs), and on minority and women owned business entities. Be very brief in your answers.
...
# """
    system_message_chat_conversation = """Assistant helps with information related to the Port Authority of New York and New Jersey infrastructure, contracts, frequently asked questions (FAQs), and on minority and women owned business entities. Be very brief in your answers.
Answer ONLY with the facts listed in the Sources below. Consider only these Sources Below. Do not include any additional knowledge. If the Sources do not support a response to the question, simply say: apologies, but I cannot answer this question based on the information I have. Do not provide any information outside the Sources. If asking a clarifying question to the user would help, ask the question.
For tabular information return it as an html table. Do not return markdown format. If the question is not in English, answer in the language used in the question.
Each source has a url followed by pipe and the actual information, always include the source url for each fact you use in the response. Use square brackets to reference the source, for example [https://somedomain.com/info1.txt]. Don't combine sources, list each source separately, for example [https://somedomain.com/info1.txt][https://somedomain.com/info2.pdf].
{follow_up_questions_prompt}
{injected_prompt}
...
"""
    query_prompt_few_shots = [
        {"role": USER, "content": "What are the business opportunities for MWBE?"},
        {"role": ASSISTANT, "content": "Show available business opportunities"},
        {"role": USER, "content": "How can I get certified?"},
        {"role": ASSISTANT, "content": "Show the certification process"},
    ]
    ...
    def log_chat(self, request_json) -> None:
        print("***Question", request_json["question"])
        print("***response", request_json["response"])
        # HOST = 'https://sanalpoccosmosdb.documents.azure.com:443/'
        # MASTER_KEY = 'VrsiPMHsdjsS6fJGYEp1m7WX0qz7bIsDKxPWmBkph7wskEYsYty5PhpSj3tr3xwmJME16lmflFaoACDb5f9IEQ=='
        # DATABASE_ID = 'OpenAIChatBot'
        # CONTAINER_ID = 'ChatHistory'
